...
public class InfiniteTerrainOffline : MonoBehaviour
{
    ...
    //android Build
    public static List<Terrain> processedTiles;
    //android Build
        ...
        for (int x = 0; x < _gridWidth; x++)
        {
            for (int y = 0; y < _gridHeight; y++)
            {
                if (!(x.Equals((_gridWidth - 1) / 2) && y.Equals((_gridHeight - 1) / 2)))
                {
                    ...
                    if (TerraLandRuntimeOffline.direction == "North" && northTerrainsList.Contains(_grid[x, y]))
                    {
                        _grid[x, y].drawHeightmap = false;
                        for (int i = 0; i < _grid[x, y].transform.childCount; i++) _grid[x, y].transform.GetChild(i)?.gameObject.SetActive(false);
                        processedTiles.Add(_grid[x, y]);
                    }
                    else if (TerraLandRuntimeOffline.direction == "South" && southTerrainsList.Contains(_grid[x, y]))
                    {
                        _grid[x, y].drawHeightmap = false;
                        for (int i = 0; i < _grid[x, y].transform.childCount; i++) _grid[x, y].transform.GetChild(i)?.gameObject.SetActive(false);
                        processedTiles.Add(_grid[x, y]);
                    }
                    else if (TerraLandRuntimeOffline.direction == "West" && westTerrainsList.Contains(_grid[x, y]))
                    {
                        _grid[x, y].drawHeightmap = false;
                        for (int i = 0; i < _grid[x, y].transform.childCount; i++) _grid[x, y].transform.GetChild(i)?.gameObject.SetActive(false);
                        processedTiles.Add(_grid[x, y]);
                    }
                    else if (TerraLandRuntimeOffline.direction == "East" && eastTerrainsList.Contains(_grid[x, y]))
                    {
                        _grid[x, y].drawHeightmap = false;
                        for (int i = 0; i < _grid[x, y].transform.childCount; i++) _grid[x, y].transform.GetChild(i)?.gameObject.SetActive(false);
                        processedTiles.Add(_grid[x, y]);
                    ...
                    //android Build
                    //if (TerraLandRuntimeOffline.direction == "North" && northTerrainsList.Contains(_grid[x, y]))
                    //    StartCoroutine(ActivateTileWithDelay(_grid[x, y].gameObject, 2.5f));
                    //else if (TerraLandRuntimeOffline.direction == "South" && southTerrainsList.Contains(_grid[x, y]))
                    //    StartCoroutine(ActivateTileWithDelay(_grid[x, y].gameObject, 2.5f));
                    //else if (TerraLandRuntimeOffline.direction == "West" && westTerrainsList.Contains(_grid[x, y]))
                    //    StartCoroutine(ActivateTileWithDelay(_grid[x, y].gameObject, 2.5f));
    ...
    //android Build
    private IEnumerator<float> ActivateTileWithDelay(GameObject go, float delay)
    {
