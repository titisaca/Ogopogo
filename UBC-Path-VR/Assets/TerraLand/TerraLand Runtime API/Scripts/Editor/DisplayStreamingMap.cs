...
namespace TerraLand
{
    public class DisplayStreamingMap : EditorWindow
    {
        ...
        [MenuItem("Tools/TerraUnity/TerraLand/Refresh")]
        public static void Init()
        {
            TFeedback.FeedbackEvent(EventCategory.UX, EventAction.Click, "Streaming_Map");
            ...
            window.titleContent = new GUIContent("Streaming Map", "Displays server's 2D map for selecting start location in world");
        }
        ...
        public void OnGUI()
        {
            ...
            if (globalSatelliteImage != null)
                EditorGUI.DrawPreviewTexture(new Rect(0, 0, position.width, position.width), globalSatelliteImage);
        ...
        {
            CalculateMouseCoords((int)key.mousePosition.x, (int)key.mousePosition.y, (int)position.width);

            GUI.color = UnityEngine.Color.gray;
            EditorGUI.HelpBox(new Rect(5, position.height - 40, 250, 16), centerLatMouse + "  " + centerLonMouse, MessageType.None);

            if (key.button == 0 && key.type == EventType.MouseDown)
                CalculateParams((int)key.mousePosition.x, (int)key.mousePosition.y, (int)position.width);

            EditorGUI.HelpBox(new Rect(position.width - 305, position.height - 40, 300, 16), "Placemark: " + centerLat + "  " + centerLon, MessageType.None);
